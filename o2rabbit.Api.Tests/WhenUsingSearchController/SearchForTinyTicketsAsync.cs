...
public class SearchForTinyTicketsAsync
{
    [Theory]
    [InlineData("", 1, 1)]
    [InlineData("aaaa", -1, 1)]
    ...
    public async Task GivenInvalidSearchOptions_ReturnsBadRequest(string searchSting, int page, int pageSize)
    {
        ...
        var response = await sut.SearchForTinyTicketsAsync(searchOptions);
    ...
    public async Task GivenValidSearchOptions_ReturnsOkResult(string searchString, int page, int pageSize)
    {
        var searchOptions = new SearchQueryParameters()
        {
    ...
    public async Task GivenTicketServiceReturnsTickets_ReturnsOkResultWithTickets()
    {
        ...
        var expected = foundTickets.Select(t => t.ToTinyTicketDto());
        ticketServiceMock.Setup(m =>
                m.SearchAsync(It.IsAny<SearchOptions>(),
                    It.IsAny<CancellationToken>()))
